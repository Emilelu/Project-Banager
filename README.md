...
- **主题与背景设置（顶部栏 🎨）**：
  ...
  - **随机壁纸背景**：默认**樱花随机二次元 · Alcy 源**（全链路跨域许可，背景与取色都可用）；可选 dmoe 樱花接口、自定义图片直链；可调**遮罩浓度**与**背景模糊**（恢复默认按钮在取色行右侧）保证页面可读，日/暗色模式使用不同遮罩底色
  ...
  - **取色**：统一通过 canvas 读取壁纸像素（Alcy 源已开放跨域权限，直接可用；图源无跨域许可时明确提示）
  - **iOS 风格壁纸聚焦动画**：弹窗打开时壁纸渐糊聚焦、关闭后渐清晰；页面启动/刷新时保持模糊直到壁纸加载完成，再由糊渐清晰入场（0.8s，跟随系统"减弱动态"设置自动禁用）
